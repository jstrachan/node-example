
# generated by: jx gitops variables
export APP_NAME='node-example'
export BRANCH_NAME='master'
export BUILD_NUMBER='6'
export DOCKERFILE_PATH='Dockerfile'
export DOCKER_REGISTRY='gcr.io'
export DOCKER_REGISTRY_ORG='jenkins-x-labs-bdd1'
export DOMAIN='35.242.181.72.nip.io'
export GIT_BRANCH='master'
export JENKINS_X_URL='http://dashboard-jx.35.242.181.72.nip.io/jstrachan/node-example/master/6'
export JX_CHART_REPOSITORY='http://jenkins-x-chartmuseum.jx.svc.cluster.local:8080'
export KANIKO_FLAGS='--snapshotMode=redo'
export MINK_AS='tekton-bot'
export MINK_IMAGE='gcr.io/jenkins-x-labs-bdd1/node-example:0.0.44'
export NAMESPACE_SUB_DOMAIN='-jx.'
export PIPELINE_KIND='release'
export PUSH_CONTAINER_REGISTRY='gcr.io'
export REPO_NAME='node-example'
export REPO_OWNER='jstrachan'
export VERSION='0.0.44'
